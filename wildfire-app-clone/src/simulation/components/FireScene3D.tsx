...
function Terrain({
  ...
  const planOverlay = usePlanOverlay(grid, planMask);
...
function usePlanOverlay(
  grid: Grid,
  planMask: Uint8Array | null | undefined
): THREE.Texture | null {
  const w = grid[0]?.length ?? 0;
  const h = grid.length;
  ...
  if (!texture || !planMask) return null;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (planMask[y * w + x]) {
        // Bright crimson with a slight pulse-like alpha for visibility.
        ctx.fillStyle = "rgba(255, 64, 60, 0.85)";
