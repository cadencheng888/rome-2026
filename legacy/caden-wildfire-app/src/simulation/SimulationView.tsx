...
import { defaultMoisture, getFuelModel } from "./fuelModels";
import { checkPrescription, defaultPrescription } from "./prescription";
import type { PrescriptionResult, PrescriptionWindow } from "./prescription";
import { buildBurnPlan } from "./burnPlan";
import type { BurnPlan } from "./burnPlan";
import { FireCanvas } from "./components/FireCanvas";
import { FireScene3D } from "./components/FireScene3D";
import { ControlPanel } from "./components/ControlPanel";
import { BurnPlanModal } from "./components/BurnPlanModal";
...
function windCardinal(x: number, y: number): string {
  // Image-space wind vector → cardinal of wind FROM (meteorological convention).
  const angle = (Math.atan2(-x, y) * 180) / Math.PI; // 0 = wind from N
...
function driftedIdleParams(prev: SimParams): SimParams {
  ...
  return {
    ...prev,
    windSpeed: drift(prev.windSpeed, 1.0, 1, 18),
    windDirX,
    windDirY,
    humidity: drift(prev.humidity, 1.5, 18, 60),
    temperature: drift(prev.temperature, 1.5, 50, 90),
  };
}
...
export default function SimulationView({
  ...
  const [showBurnPlan, setShowBurnPlan] = useState(false);
  ...
  const breakdown: RiskBreakdown = useMemo(() => {
    if (!grid) {
      // Compute behavior at current params with zero slope as a placeholder
      // so the panel still shows live numbers before terrain loads.
      return calculateRiskBreakdown([[]] as unknown as Grid, params, elevation);
    }
    return calculateRiskBreakdown(grid, params, elevation);
  }, [grid, params, elevation]);
  const riskScore = breakdown.score;
  const burning = grid ? isAnyBurning(grid) : false;

  const prescription: PrescriptionWindow = useMemo(
    () => defaultPrescription(getFuelModel(params.fuelModelCode)),
    [params.fuelModelCode],
  );

  const rxResult: PrescriptionResult = useMemo(
    () =>
      checkPrescription(
        {
          tempF: params.temperature,
          rh: params.humidity,
          midflameWindMph: params.windSpeed,
          // Smoke fields are not modeled — pass conservative placeholders so
          // they generate warnings, not silent passes.
          transportWindMph: 12,
          mixingHeightFt: 1800,
          ventilationIndex: 4500,
          daysSinceRain: 5,
          fm: params.fuelMoisture,
          behavior: breakdown.behavior,
          rosChHr: breakdown.rosChHr,
        },
        prescription,
      ),
    [params, prescription, breakdown.behavior, breakdown.rosChHr],
  );

  const burnPlan: BurnPlan = useMemo(() => {
    const fm = getFuelModel(params.fuelModelCode);
    return buildBurnPlan({
      locationName:
        locations.find((l) => l.id === selectedId)?.name ?? selectedId,
      fuelModel: fm,
      moisture: params.fuelMoisture,
      prescription,
      behavior: breakdown.behavior,
      rosChHr: breakdown.rosChHr,
      rxResult,
      ...
      rh: params.humidity,
      midflameWindMph: params.windSpeed,
      windDirCardinal: windCardinal(params.windDirX, params.windDirY),
      slopePct: breakdown.meanSlopePct,
      area_ac: 250, // TBD — UI-driven in a follow-up
      preparedBy: "[Burn Boss to fill]",
    });
  }, [params, locations, selectedId, prescription, breakdown, rxResult]);
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100vw" }}>
      ...
      <ControlPanel
        ...
        onShowBurnPlan={() => setShowBurnPlan(true)}
      />
      {showBurnPlan && (
        <BurnPlanModal plan={burnPlan} onClose={() => setShowBurnPlan(false)} />
      )}
    </div>
  );
}
